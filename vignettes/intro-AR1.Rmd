...
Acknowledgments: Marie Denis (CIRAD)
...
set.seed(1859)
ar1xar1.sim.mv <- function(N, R, C, rho.r, rho.c, sigma2){
  stopifnot(R > 1, C > 1, abs(rho.r) <= 1, abs(rho.c) <= 1, sigma2 >= 0)
  ## ... TODO ...
}
```
...
X2 <- ar1xar1.sim.mv(N, R=2, C=500, rho.r=0, rho.c, sigma2)
prettyPrintSummary(apply(X2, 3, function(Xi){
  acf(Xi[1,], lag.max=1, type="correlation", plot=FALSE)$acf[2]
}))
```
...
X2 <- ar1xar1.sim.mv(N, R=500, C=2, rho.r, rho.c=0.0, sigma2)
prettyPrintSummary(apply(X2, 3, function(Xi){
  acf(Xi[,1], lag.max=1, type="correlation", plot=FALSE)$acf[2]
}))
```
...
X2 <- ar1xar1.sim.mv(N, R, C, rho.r, rho.c, sigma2)
prettyPrintSummary(apply(X2, 3, function(Xi){
  acf(Xi[1,], lag.max=1, type="correlation", plot=FALSE)$acf[2]
}))
prettyPrintSummary(apply(X2, 3, function(Xi){
