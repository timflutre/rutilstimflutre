...
suppressPackageStartupMessages(library(breedR)) # on GitHub
nb.threads <- 3 # 1
```
...
Let's take as an example multiple genotypes phenotyped several years in a row (no intra-year replicate):
...
* $y_{iq}$: phenotypic value of the $i$-th genotype the $q$-th year

* $\beta_q$: effect of the $q$-th year, modeled as a fixed effect

* $g_i$: genotypic value of the $i$-th genotype, modeled as a random variable such that $\boldsymbol{g} \sim \mathcal{N}(\boldsymbol{0}, \sigma_g^2 G)$

* $\epsilon_{iq}$: error corresponding to the $i$-th genotype the $q$-th year, modeled as $\boldsymbol{\epsilon} \sim \mathcal{N}(\boldsymbol{0}, \sigma_\epsilon^2 \text{Id})$
...
\[
\mathcal{M}_1: \; \forall q \in \{1,\ldots,Q\}, i \in \{1,\ldots,I\}, \; y_{qi} = \beta_q + g_i + \epsilon_{qi}
\]

It can also be rewritten as $\boldsymbol{y} = X \boldsymbol{\beta} + Z \boldsymbol{g} + \boldsymbol{\epsilon}$ where $X$ and $Z$ are appropriate incidence matrices.

## Model 2

Take into account covariances between years.

Likelihood:

\[
\mathcal{M}_2: \; Y = X B + Z G + E
\]
...
year.ids <- paste0("Y", 2011:(2011+Q-1))
sigma.g <- 3
sigma.epsilon <- 3
(h2 <- sigma.g^2 / (sigma.g^2 + sigma.epsilon^2))
```

Make a data frame with the appropriate structure:
```{r make_df}
dat <- data.frame(geno=rep(geno.names, Q),
                  year=rep(year.ids, each=I),
                  pheno=NA)
```
...
Simulate the explanatory and error variables, assuming for simplicity an identity matrix for $G$:
```{r simul_var}
beta <- matrix(c(50, rmvnorm(n=1, mean=rep(0, Q-1), sigma=20 * diag(Q-1))))
G <- diag(I) # for simplicity
dimnames(G) <- list(geno.names, geno.names)
g <- t(rmvnorm(n=1, mean=rep(0, I), sigma=sigma.g^2 * G))
...
g.hat.inla.MF2 <- setNames(fit.inla.MF2$summary.random$geno.id[,"mean"],
...
g.hat.breedR <- setNames(as.vector(ranef(fit.breedR)$geno),
                         names(ranef(fit.breedR)$geno))
var.g.hat.breedR <- attr(ranef(fit.breedR)$geno, "se")^2
sigma.epsilon.hat.breedR <- sqrt(fit.breedR$var["Residual",
                                                "Estimated variances"])
sigma.g.hat.breedR <- sqrt(fit.breedR$var["geno",
                                          "Estimated variances"])
...
x.lim <- max(abs(g.hat.breedR))
par(mar=c(5,6,4,1))
plot(x=g.hat.breedR, y=1:length(g.hat.breedR),
     xlim=c(-x.lim, x.lim),
     yaxt="n", ylab="")
axis(side=2, at=1:length(g.hat.breedR), labels=names(g.hat.breedR),
     las=1)
abline(v=0, lty=2)
```
