...
stopifnot(compareVersion("0.168.1",
...
if(! file.exists(p2f)){
  globmod.ml <- lme4::lmer(formula=as.formula(form),
                           data=dat.noNA,
                           na.action=na.fail,
                           REML=FALSE)
  ## st <- system.time(
...
if(cor.spat.het){
  ...
  bestmod.ml <- lme4::lmer(formula=best.form,
                           data=dat.noNA,
                           na.action=na.fail,
                           REML=FALSE)
  bestmod.reml <- lme4::lmer(formula=formula(bestmod.ml),
...
plotResidualsBtwYears(df=fit.all, colname.res="scl.cond.res",
                      years=c("2011","2012"), blocks=c("A","A"))
```
