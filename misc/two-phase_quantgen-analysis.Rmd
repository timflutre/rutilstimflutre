...
stopifnot(compareVersion("0.168.2",
...
```{r kriging}
fix.preds.corr.spat <- out$all.preds[-grep("\\(*\\|*\\)|year", out$all.preds)]
dat2 <- correctSpatialHeterogeneity(dat=dat,
                                   response=response,
...
all.preds.as.fix <- gsub("\\(1 \\| |\\)", "", out$all.preds)
...
```{r H2}
reps.geno.year <- tapply(dat.noNA[[response]],
