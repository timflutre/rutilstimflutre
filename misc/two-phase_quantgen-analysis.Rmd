...
stopifnot(compareVersion("0.165.0",
...
* $I$: number of genotypes (from a panel, as well as a control)

* $Q = J + K$: number of known, non-genetic factors and covariates

    * $J$: number of blocks (one panel genotype per block, several controls per block)
...
* $\boldsymbol{g}$: $I$-vector of genotypic values, modeled as random
    ...
    * $H^2 = \frac{\sigma_g^2}{\sigma_g^2 + \sigma^2 / rep}$: broad-sense heritability for the means of panel genotypes
...
* use weights in phase 2 in the presence of missing data? $\rightarrow$ TBD
...
TODO: change the genetic architecture so that there are few large effects and many (very) small effects
...
```{r simul_phenos_spatial-het_design-mat}
...
Draw several fields with spatial heterogeneity (set very high correlations between ranks and between locations, as the empirical values in the sample will be lower, see below):
...
```{r kriging}
fix.preds.corr.spat <- all.preds[-grep("\\(*\\|*\\)|year", all.preds)]
cor.spat.het <- FALSE
if(cor.spat.het){
...
Re-fit the best model using the data corrected for spatial heterogeneity:
```{r bestmod_refit_spatial_correct}
if(cor.spat.het){
  print(best.form <- gsub(response, paste0(response, ".csh"), best.form))
  response <- paste0(response, ".csh")
  bestmod.ml <- lmer(formula=best.form,
                     data=dat.noNA,
                     na.action=na.fail,
                     REML=FALSE)
  bestmod.reml <- lmer(formula=formula(bestmod.ml),
...
```{r}
all.preds.as.fix <- gsub("\\(1 \\| |\\)", "", all.preds)
## all.preds.as.fix <- all.preds.as.fix[-grep("geno:year", all.preds.as.fix)]
best.form.as.fix <- paste0(response, " ~ 1 + ",
                           paste(all.preds.as.fix, collapse=" + "))
...
Look at the empirical variogram, and fit a Matérn model:
...
for(year in levels(fit.all$year)){
  ...
  print(plot(resid.vg, resid.vg.fit, main="", col="blue",
             key=list(space="top", lines=list(col="blue"),
                      text=list(paste0(response, ": fit of Matérn model in ",
                                       year)))))
}
```
