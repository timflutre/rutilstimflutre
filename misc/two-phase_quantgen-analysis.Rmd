...
* use BLUEs or BLUPs of $\boldsymbol{g}$ in phase 2? $\rightarow$ BLUPs if unbalanced data set

* use weights in phase 2 in the presence of missing data? $\rightarow$ TBD
...
M.d.c <- matrix(NA, nrow=N, ncol=P, dimnames=dimnames(X))
for(i in 1:N){
  ...
  print(plot(vg, vg.fit, main="", col="blue",
             key=list(space="top", lines=list(col="blue"),
                      text=list(paste0(response,
                                       ": fit of Matérn model on controls in ",
                                       year)))))
