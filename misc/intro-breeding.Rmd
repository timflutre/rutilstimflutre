...
plotRegMidparentsOffsprings <- function(mu.0, sigma.02, h2, y, y.e,
  ...
  title(main=bquote(bold(Linear~regression~of~offsprings~on~average~parents)~(h^2==.(h2)*","~mu^(s)==.(format(mu.s, digits=2))*","~CV[g]==.(format(CV.g, digits=2))*","~alpha==.(format(100*alpha, digits=2))*"%"*","~i==.(format(i, digits=3)))))
