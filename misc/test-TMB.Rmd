...
if(FALSE){
  ...
  cbind("truth"=c("mu"=mu, "log(sigma)"=log(sigma)), "estim"=fit$par)
  out <- tmbstan(f, chains=1)
  traceplot(out, pars=names(f$par), inc_warmup=TRUE)
  pairs(out, pars=names(f$par))
  plot(out, pars=names(f$par))
}
```
...
Type objective_function<Type>::operator() ()
{
  ...
  SIMULATE {
    ...
    vector<Type> u_sim(q);
    vector<Type> e_sim(n);
    VECSCALE_t<MVNORM_t<Type>> f_u_sim = VECSCALE(mvn_u_cor, vec_sigma_u);
    f_u_sim.simulate(u_sim);
    VECSCALE_t<MVNORM_t<Type>> f_e_sim = VECSCALE(mvn_y_cor, vec_sigma_e);
    f_e_sim.simulate(e_sim);
    y_sim = X * beta + Z * u_sim + e_sim;
    REPORT(y_sim);
  }
...
```{r}
f <- MakeADFun(data=list(y=y, X=X, Z=Z, A=A, Id_n=Id_n, nbReps=nbBlocks),
               parameters=list(beta=rep(0, p), u=rep(0, q),
                               log_sigma_u=log(1), log_sigma_e=log(1)),
               random="u", DLL=dllID)
fit <- nlminb(f$par, f$fn, f$gr)
allPars_estim <- f$env$last.par.best
length(allPars_estim) # includes beta, u and the variances

## test with one simulation:
new_sim <- f$simulate(par=allPars_estim, complete=TRUE)
names(new_sim) # y, X, Z, A, Id_n, nbReps and y_sim
summary(y)
summary(new_sim$y_sim)
op <- par(mfrow=c(1,2))
hist(y, breaks=20); hist(new_sim$y_sim, breaks=20)
par(op)

## https://kaskr.github.io/adcomp/Simulation.html
## several simulations:
list_allPars_estim <- list()
for(param in c("beta", "u", "log_sigma_u", "log_sigma_e")){
  idx <- grep(paste0("^",param,"$"), names(allPars_estim))
  list_allPars_estim[[param]] <- allPars_estim[idx]
}
system.time(
    sim <- replicate(50, {
      simdata <- f$simulate(par=allPars_estim, complete=TRUE)
      simdata$y <- simdata$y_sim
      simdata$y_sim <- NULL
      f2 <- MakeADFun(data=simdata, parameters=list_allPars_estim,
                      random="u", DLL=dllID, silent=TRUE)
      nlminb(f2$par, f2$fn, f2$gr)$par
    })
)
rowMeans(sim)
allPars_estim[c(1,2,3, 54,55)]
apply(sim, 1, summary)

df <- data.frame(estimate=c(sim),
                 parameter=c("beta1","beta2","beta3","log_sigma_u","log_sigma_e"))
ggplot(df) +
  aes(x=estimate) +
  geom_histogram() +
  theme_bw() +
...
Type objective_function<Type>::operator() ()
{
  ...
  SIMULATE {
    ...
    f_u_sim.simulate(u_sim);
    GMRF_t f_e_sim(Rinv);
//    GMRF_t<SparseMatrix<Type>> f_e_sim(Rinv);
    f_e_sim.simulate(e_sim);
    y_sim = X * beta + Z * u_sim + e_sim;
    REPORT(y_sim);
  }
...
```{r}
listParams <- list(beta=rep(0, p), u=rep(0, q),
                   log_sigma_u=log(1), log_sigma_e=log(1))
f <- MakeADFun(data=list(y=y, X=X, Z=Z, A=A, Id_n=Id_n, nbReps=nbBlocks),
...
Using the explicit formula of the likelihood with sparse covariance matrices (eq 26 of Zhu and Wathen, 2018).
...
if(FALSE){
  ...
  Y_v2 <- invvec(X_2 %*% b, 2) + invvec(Z_2 %*% u, 2) + invvec(e, 2)
  stopifnot(all(Y_v2[,1] == Y[,1]),
            all(Y_v2[,2] == Y[,2]))
}
dat2$y <- y
```
...
```{r}
system.time(
    f_v1 <- MakeADFun(data=list(Y=Y, X_1=X_1, Z_1=Z_1, A=A, Id_n1=diag(n_1)),
                      parameters=list(
                          B=matrix(0,p,d),
                          U=matrix(0,q,d),
...
if(FALSE){
  ...
  pairs(out, pars=names(f$par))
  plot(out, pars=names(f$par))
  print(out, pars=c("unconstr_cor_U","unconstr_cor_E"))
}
```
...
template<class Type>
void invvec(const vector<Type>& x, int ncol, matrix<Type>& mat)
{
  int nrow = (int) x.size() / ncol;
  int idx = 0;
...
Type objective_function<Type>::operator() ()
{
  DATA_VECTOR(y);                    //   n_2   x    1
  DATA_MATRIX(X_2);                  //   n_2   x    dp
  DATA_MATRIX(Z_1);                  //   n_1   x    q
  DATA_MATRIX(A);                    //    q    x    q
  DATA_MATRIX(Id_n1);                //   n_1   x   n_1
  PARAMETER_VECTOR(b);               //    dp   x    1
  PARAMETER_ARRAY(U);                //    q    x    d
  PARAMETER_VECTOR(log_sd_U);        //    d    x    1
  PARAMETER_VECTOR(unconstr_cor_U);  // lower triangle, filled row-wise
  PARAMETER_VECTOR(log_sd_E);        //    d    x    1
  ...
  MVNORM_t<Type> g_y(Id_n1);         //   n_1   x   n_1
  SEPARABLE_t< VECSCALE_t<UNSTRUCTURED_CORR_t<Type> > , MVNORM_t<Type> > h_Y(f_y, g_y);
  matrix<Type> Y_mat(n_1, d);
  invvec(y, d, Y_mat);
  array<Type> Y(n_1, d);
  Y = Y_mat;
  matrix<Type> X_1B(n_1, d);
  vector<Type> X_2b = (X_2 * b).col(0);
  invvec(X_2b, d, X_1B);
  matrix<Type> M(Y.rows(), d);
  M = X_1B + Z_1 * U.matrix();
  nll += h_Y(Y - M.vec());
...
```{r}
system.time(
    f_v2 <- MakeADFun(data=list(y=y, X_2=X_2, Z_1=Z_1, A=A, Id_n1=diag(n_1)),
                      parameters=list(
                          b=rep(0,d*p),
                          U=matrix(0,q,d),
                          log_sd_U=rep(log(1),d),
...
rbind("truth"=b,
      ...
      "TMB_v1"=fits_v1$gr$par[grep("B", names(fits_v1$gr$par))],
      "TMB_v2"=fits_v2$gr$par[grep("b", names(fits_v2$gr$par))])

rbind("truth"=c(setNames(c(var_U1, cor_U, var_U2),
                         c("var_U_1", "cor_U", "var_U_2")),
                setNames(c(var_E1, cor_E, var_E2),
                         c("var_E_1", "cor_E", "var_E_2"))),
      "TMB_v1"=c(exp(fits_v1$gr$par[grep("log_sd_U", names(fits_v1$gr$par))[1]])^2,
                 repCor$Cor_U[1,2],
                 exp(fits_v1$gr$par[grep("log_sd_U", names(fits_v1$gr$par))[2]])^2,
                 exp(fits_v1$gr$par[grep("log_sd_E", names(fits_v1$gr$par))[1]])^2,
                 repCor$Cor_E[1,2],
                 exp(fits_v1$gr$par[grep("log_sd_E", names(fits_v1$gr$par))[2]])^2),
      "TMB_v2"=c(exp(fits_v2$gr$par[grep("log_sd_U", names(fits_v2$gr$par))[1]])^2,
                 repCor$Cor_U[1,2],
                 exp(fits_v2$gr$par[grep("log_sd_U", names(fits_v2$gr$par))[2]])^2,
                 exp(fits_v2$gr$par[grep("log_sd_E", names(fits_v2$gr$par))[1]])^2,
                 repCor$Cor_E[1,2],
                 exp(fits_v2$gr$par[grep("log_sd_E", names(fits_v2$gr$par))[2]])^2))
```
...
```{r}
plot(summary(sdrep_v1, select="random")[1:q,1], U[,1],
     xlab="predicted u", ylab="true u", las=1,
     main="Random effects U[,1]")
abline(h=0, v=0, a=0, b=1, lty=2)
abline(lm(U[,1] ~ summary(sdrep_v1, select="random")[1:q,1]), col="red")

plot(summary(sdrep_v2, select="random")[-(1:q),1], U[,2],
     xlab="predicted u", ylab="true u", las=1,
     main="Random effects U[,2]")
abline(h=0, v=0, a=0, b=1, lty=2)
abline(lm(U[,2] ~ summary(sdrep_v2, select="random")[-(1:q),1]), col="red")
```
