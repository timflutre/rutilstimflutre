...
Interestingly, even though the SNPBLUP model assumes a single variance for the SNP effects, we can then estimate a SNP-specific variance for the effect of any SNP $j$ as in Zhang et al (2010):

$\hat{\sigma}_{u,j}^2 = \hat{u}_j^2 \, 2 p_j (1 - p_j)$

<!-- The $t$-statistic for "$H_0: u_j = 0$" is: $\hat{u}_j / \hat{\sigma}_{u,j}^2$. -->
...
In the SNPBLUP model, the BLUPs of the SNP effects, $\hat{\boldsymbol{u}}$,  as well as its variance, $Var(\hat{\boldsymbol{u}})$, can be obtained by the inversion of the coefficient matrix of the MMEs: ...

The $t$-statistic for "$H_0: u_j = 0$" is: $\hat{u}_j / \sqrt{Var(\hat{u}_j)}$.

## SNP effects as fixed

For each SNP $j$: $\boldsymbol{y} = \boldsymbol{z}_j b_j + \boldsymbol{a} + \boldsymbol{e}$ with $\boldsymbol{a} \sim \mathcal{N}(\boldsymbol{0}, \sigma_a^2 G)$ and $\boldsymbol{e} \sim \mathcal{N}(\boldsymbol{0}, \sigma_e^2 Id)$.
...
The $t$-statistic for "$H_0: b_j = 0$" is: $\hat{b}_j / \sqrt{Var(\hat{b}_j)}$.
