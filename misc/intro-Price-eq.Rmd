...
In quantitative genetics since Fisher, the parental phenotype $z$ is classically decomposed into a transmissible part, $g$, and a non-transmissible part, $\epsilon$ with $z = g + \epsilon$ and $\text{E}(\epsilon) = 0$, so that $z' = g$.
...
\begin{align}
w &= \beta_{wz} (g + \epsilon) + \delta \\
 &= \beta_{wg} g + \gamma
 \end{align}

where $\beta_{wg} := \beta_{wz}$ and $\gamma := \beta_{wz} \epsilon + \delta$.
