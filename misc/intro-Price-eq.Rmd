...
Following the vocabulary most often used in the references at the end of this document, a **set** is made of **entities**, each entity has a **type** and a **property**, and several entities can have the same type and property.
The focus is on types, hence the references below do not index entities, and I follow this here.

**In the ancestral set:** (i.e., before selection)

- each entity has a **type**, indexed by $i \in \{1,\ldots,I\}$, and a **property**, noted $z_i$ so that it is the average property over all entities of type $i$ <!-- (if entities of type $i$ were indexed by $j \in \{1,\ldots,n_i\}$ with $n_i$ being the number of entities of type $i$ in this set, one would write $z_i := (1/n_i) \sum_{j=1}^{n_i} z_{ij}$) -->
...
In the scheme below, each disk is an entity, the numbers indicate the types and the color refer to the property.
...
Importantly, Zhang and Hill (2010) highlight that the Price equation holds only if the character value changes after the selection, i.e., it is valid only when the two processes (selection and transmission) occur independently such that their effects are additive.

<br>

# Robertson-Price identity

A very simple case consists in applying the Price equation to a single episode of selection within a generation, e.g., due to survival only, so that the second term of the equation can be ignored.
Let us hence assume that the ancestral set consists of all individuals in the population and the descendant set consists only in the survived individuals.
Moreover, let $z$ denote the phenotypic values of the trait of interest in the ancestral set.

In this case, the Robertson-Price identity simply is:

\[
S = cov(w, z)
\]

where $S$ corresponds to the **selection differential** (see below).

As explained in Stinchcombe et al (2014), it can be generalized to multivariate form.
Moreover, it includes the net effects of direct and indirect selection on the traits of interest, whether correlated traits were measured or even included in the analysis, which is not the case of the (multivariate) breeder's equation (see below).

<br>

# Robertson's secondary theorem

Let us now assume that the ancestral set consists in potential parents and the descendant set consists in their offsprings.
...
As a result, Price's equation applied to this mapping expresses the **selection differential** (also known as the Robertson-Price identity, see above):
...
Zhang and Hill (2010) show that the Price equation can be used to derive the change, not only in trait mean ($\mu$) but also in *variance*, both genetic ($V_{Am}$) and environmental ($V_E$; also called "residual variance", i.e., variance of phenotypic value given genotype).
...
- Zhang X-S, Hill WG. 2010. Change and maintenance of variation in quantitative traits in the context of the Price equation. Theoretical Population Biology. 77(1):14–22. doi:10.1016/j.tpb.2009.10.004
...
- Stinchcombe JR, Simonsen AK, Blows MarkW. 2014. Estimating uncertainty in multivariate responses to selection: brief communication. Evolution. 68(4):1188–1196. doi:10.1111/evo.12321.
