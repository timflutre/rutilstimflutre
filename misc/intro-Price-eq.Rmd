...
In quantitative genetics since Fisher, the parental phenotype $z$ is classically decomposed into a transmissible part, $a$, and a non-transmissible part, $\epsilon$ with $z = a + \epsilon$ and $\text{E}(\epsilon) = 0$, so that $z' = a$.
...
R &= \bar{z}_{\text{offsprings}} - \bar{z}_{\text{potential parents}} \\
 &= Cov(w, a + \epsilon) - \text{E}(w \epsilon) \\
 &= Cov(w, a) + Cov(w, \epsilon) - Cov(w, \epsilon) \\
 &= Cov(w, a)
\end{align}
...
\begin{align}
w &= \beta_{wz} (a + \epsilon) + \delta \\
 &= \beta_{wa} a + \gamma
 \end{align}

where $\beta_{wa} := \beta_{wz}$ and $\gamma := \beta_{wz} \epsilon + \delta$.

Importantly, let us also assume that $Cov(a, \gamma) = 0$, which usually is reasonable for artificial selection but much less so for natural selection.
...
R &= Cov(w, a) \\
 &= \beta_{wg} Var(a) \\
 &= \beta_{wz} Var(a) \frac{Var(z)}{Var(z)} \\
 &= \frac{Var(a)}{Var(z)} \, Cov(w, z) \\
 &= h^2 \, S
\end{align}
