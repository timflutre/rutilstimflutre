...
* first $C / C_{11} = C_{11} - C_{12} C_{21}^{-1} C_{21}$, and then $C^{11} := (C / C_{11})^{-1}$
...
Proving this requires to invert $C$, the coefficient matrix of the MMEs, and to retrieve its top-left submatrix, noted $C^{11}$.
As $C$ is a block matrix, its inverse is one, too, and formulas exist:
