...
output:
  ...
  pdf_document:
    ...
    number_sections: TRUE
abstract: |
    This documents introduces the mixed model equations (MMEs) for variance-component linear mixed models (LMMs).
---
...
The fixed effects are used to model the mean of $\boldsymbol{y}$ and the random effects are used to model its variance.
...
After integrating out the $\boldsymbol{u}$ (one also speaks of *marginalizing* them out):
...
By definition, the likelihood noted $\mathcal{L}$ is the probability density of the data given the parameters, $p(\boldsymbol{y} | \boldsymbol{\beta}, \theta)$, but most importantly it is a function of the parameters:

\[
\mathcal{L}(\boldsymbol{\beta}, \theta) = (2 \pi)^{-\frac{n}{2}} \, |V(\theta)|^{-\frac{1}{2}} \, \exp \left( - \frac{1}{2}  (\boldsymbol{y} - X \boldsymbol{\beta})^T V(\theta)^{-1} (\boldsymbol{y} - X \boldsymbol{\beta}) \right)
\]

where $|M|$ denotes the determinant of matrix $M$.

In practice the log-likelihood is maximized as it is easier to compute and delivers the same results (the log is monotonous):
\[
...
Given that the expectation of $\boldsymbol{y}$ is $X \boldsymbol{\beta}$ and that its variance $V(\theta)$ is considered known here, one can maximize the likelihood (equivalent of the generalized least squares) to estimate $\boldsymbol{\beta}$:

\[
\boldsymbol{\hat{\beta}} = (X^T V^{-1} X)^{-1} X^T V^{-1} \boldsymbol{y}
\]

eventually using a generalized inverse if $X$ is not full rank.
...
It is useful to re-write this by separating, as far as possible, the terms depending on $\boldsymbol{\beta}$ and those depending on $\boldsymbol{u}$:
...
Then, one computes the first partial derivatives (called the *score* function when the derived function is a likelihood):
\begin{align}
...
where the $(p+q) \times (p+q)$ matrix on the left-hand side is named the coefficient matrix, noted $C$ and partitioned into four submatrices $C_{11}$, $C_{12}$, $C_{21}$ and $C_{22}$.

## BLUEs and BLUPs

Developping these equations:
...
Inserting in the first equation the value of $\hat{\boldsymbol{u}}$ from the second, we get:
...
Still, the formulas above require the inverse of $V$.
Hopefully, it is possible to show that $V^{-1} = R^{-1} - R^{-1} Z W Z^T R^{-1}$ where $W = C_{22}^{-1} = (Z^T R^{-1} Z + G^{-1})^{-1}$

Indeed:

\begin{align}
V \, (R^{-1} - R^{-1} Z W Z^T R^{-1}) &= (R + Z G Z^T) \, (R^{-1} - R^{-1} Z W Z^T R^{-1}) \\
    &= \text{Id} + Z G Z^T R^{-1} - Z W Z^T R^{-1} - Z G Z^T R^{-1} Z W Z^T R^{-1} \\
    &= \text{Id} + Z G Z^T R^{-1} - Z (\text{Id} + G Z^T R^{-1} Z) W Z^T R^{-1} \\
    &= \text{Id} + Z G Z^T R^{-1} - Z G (G^{-1} + Z^T R^{-1} Z) W Z^T R^{-1} \\
    &= \text{Id} + Z G Z^T R^{-1} - Z G W^{-1} W Z^T R^{-1} \\
    &= \text{Id}
\end{align}

Usually, $R^{-1}$ has a structure allowing it to be easily computed.
Moreover, $W$, i.e., the inverse of $C_{22}$, is $q \times q$ instead of $n \times n$ making it easier to invert than $V$ which is $n \times n$.

As a result, the formula for the BLUP can also be simplified:

\begin{align}
\hat{\boldsymbol{u}} &= G Z^T V^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= G Z^T (R^{-1} - R^{-1} Z W Z^T R^{-1}) (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= G (Z^T R^{-1} - Z^T R^{-1} Z W Z^T R^{-1}) (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= G (\text{Id} - Z^T R^{-1} Z W) Z^T R^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= G (W^{-1} - Z^T R^{-1} Z) W Z^T R^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= G (Z^T R^{-1} Z + G^{-1} - Z^T R^{-1} Z) W Z^T R^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= (G Z^T R^{-1} Z + \text{Id} - G Z^T R^{-1} Z) W Z^T R^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}}) \\
    &= W Z^T R^{-1} (\boldsymbol{y} - X \hat{\boldsymbol{\beta}})
\end{align}
