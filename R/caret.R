...
##' @param x current predictors used to fit the model, i.e genotypic matrix in Joinmap format with genotypes in rows and markers in columns
##' @param y current outcome used to fit the model, i.e trait to study in numeric format with genotype names
##' @param wts optional instance weights
##' @param param data frame with current tuning parameter values
##' @param lev class levels of the outcome (or NULL in regression)
##' @param last logical for whether the current fit is the final fit
##' @param weights ?
##' @param classProbs logical for whether class probabilities should be computed
...
##' @param nperm number of permutations to be done is tuneThreshold=FALSE
...
##' @param plot logical, default is FALSE. If method is MIM, 2 arguments are needed.
...
caretFitQtl <- function (x, y, wts, param, lev, last, weights, classProbs,
  ...
  if (!is.null(cross.geno.prob)) {
    ...
    for (i in 1:length(list_locus)) {
      tmp <- c(paste0(list_locus[i], "@AC"), paste0(list_locus[i],"@AD"),
               paste0(list_locus[i], "@BC"), paste0(list_locus[i],"@BD"))
    ...
    if (all(!is.na(qtl.df$linkage.group))) {
      ...
      if(!is.na(alpha[2])){
        ...
        rownames(summary.fit)[-1] <- substr(rownames(summary.fit)[-1], start=6, stop=nchar(rownames(summary.fit)[-1]))
